...
class AuthController
{
        ...
        if (isset($_POST['submit'])) {      
            ...
            // 6. Jika data hasil query ditemukan:
                if (mysqli_num_rows($row)){
                    #$query2 = "INSERT INTO mahasiswa VALUES ('','nim','id_pendaftaran','nama','jurusan','password')";
    ...
    private function generateNIM($id_pendaftaran){
        ...
        $query5 = "SELECT * FROM pendaftaran WHERE id = '$id'";
        $result5 = mysqli_query($conn, $query5);
        $tahunmasuk = DateTime::createFromFormat("Y", "2024");
        
        $data_pendaftaran = $result5->fetch_assoc();

        if (mysqli_num_rows($data_pendaftaran)){
            $kode_jurusan = $data_pendaftaran['jurusan'];
            $kode_jurusan = $this->getJurusan($kode_jurusan);
            
            if ($kode_jurusan != 0) {
                $nim = str_pad($kode_jurusan + $tahunmasuk + $data_pendaftaran["id"]);
